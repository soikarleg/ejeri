...
$devisestimatif = strEncoding("Bordereau de dépôt");
...
define('N', $n);
define('FONT', 9);
define('VOTRECONTACT', 'Information bordereau');
...
$data_bordereau = "and annee = '2024' and mois = '01'";
...
class MyDevis extends FPDF
{
  function Header()
  {
    ...
    $this->Cell(0, SAUT,   'Edition du ' . LIEUDATE, '', 1, 'R');
    $this->Ln(28);
    // $this->SetFont('Nunito', 'B', 14);
    $this->Cell(0, SAUT + 4, 'vrt_hhh_xx', '', 1, 'R');
    $y = $this->GetY();
    define('Y', $y);
    $this->SetFont('Nunito', 'B', 10);
    $this->SetY($y - 5);
  ...
  function Lignes($data_bordereau)
  {
    $conn = new connBase();
    $req_reglement = "select *,SUM(montant) as total from reglements where cs = '" . SEC . "' " . DATA . " group by bordereau order by time_maj desc";
    $regl = $conn->allRow($req_reglement);

    $this->SetFont('Nunito', 'B', 10);
    $this->Cell(160, SAUT + 2,  'Bordereau', 'B', 0, 'L');
    $this->Cell(30, SAUT + 2,  'Montant', 'B', 1, 'R');
    $this->Ln(4);

    foreach ($regl as $l) {
      $this->SetFont('Nunito', '', 10);
      $desi = mb_convert_encoding($l['bordereau'], 'ISO-8859-1');
      ...
      $this->SetX(175);
      $q = $l['montant'] === 0.00 ? "" : Dec_2($l['montant']);
...
$pdf->AddPage();
//$pdf->Adresse();
$pdf->Lignes($data_bordereau);



$fichier = 'Bordereau_' .  date('dmY_His');
