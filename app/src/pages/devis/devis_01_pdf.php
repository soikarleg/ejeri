...
define('LIEUDATE', ('A ' . $c_ville . ', le ' . $jour . '/' . $mois . '/' . $annee));
...
define('SEC', $secteur);
define('PIECE', $numero);
define('ETAB', strEncoding('Etition du '));
define('DE', date('d/m/Y'));
...
class MyDevis extends FPDF
{
  ...
  function Lignes($numero)
  {
    ...
    $y = $this->GetY();
    $this->SetFillColor(253);
    $this->RoundedRect(10, $y, 105, 35, 2, 'DF');
    $this->SetY($y + 36);
    $sans_acompte = mb_convert_encoding('-', 'ISO-8859-1');
