...
?>
<script>
  getIframePDF('../src/pages/factures/factures_02_pdf.php?numero=<?= $numero ?>&idcli=<?= $idcli ?>', 'pdf');
</script>
<div class="row">
  ...
  <div class="col-md-4">
    ...
    <!---->
    <p class="btn btn-mag pointer mb-2" onclick="getIframePDF('../src/pages/factures/factures_02_pdf.php?numero=<?= $numero ?>&idcli=<?= $idcli ?>','pdf')">Réafficher le document</p>
    <?php
    $commentaire;
    $designation;
    $titre;
    ...
    VALUES ('$idcli','$datefact_entete','$dateche_entete','$factav','$secteur','$numero','$nomcli','$commercial','$devref','$titre','$apayer','$tva','$commentaire','$jen','$men','$aen','non','$jec','$mec','$aec')";
      ...
      $conn->insertLog('Création facture/avoir', $iduser, 'Facture/Avoir ' . $numero . ' pour ' . $nomcli . '  ' . $apayer);
    } else {
    ...
    ?>
    <p class="text-bold mb-2">Envoyer la facture par email au client <span><?= $la_facture_existe ?></span></p>
    <?php
    if ($client['email']) {
      ...
      if ($secteur = "C4X") {
        $complement = "Vos chèques devrons être envoyés au 3, place de l'Eglise 45740 Lailly-en-Val.\n";
      }

      $message_type = "Bonjour $nom_client,\n\nVeuillez trouver votre facture N° $numero en pièce jointe.\nMerci de noter le numéro de facture sur l'ordre de votre virement ou au dos de votre chèque.\n$complement Nous restons à votre écoute pour répondre à toutes vos questions.\n\nCordialement,\n$nom_secteur\n" . $mail_secteur['telephone'] . "\n$email_secteur\n\n$nom_entreprise";
    ?>
      <form action="" id="email_devis">
        ...
        <div class="text-right">
          ...
          <input name="Envoyer" type="button" class="btn btn-mag-n text-primary" value="Envoyer la facture" onclick="ajaxForm('#email_devis', '../src/pages/factures/facture_envoi_mail.php', 'sous-target', 'attente_target');" />
        </div>
      </form>
