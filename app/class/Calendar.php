...
class Calendar
{
    ...
    public function asMonthView($date = false, $color = false)
    {
        ...
        foreach ($this->getDays() as $index => $day) {
            ...
            $calendar .= '<th class="px-3 py-0 cal-th cal-th-' . $index . ' cal-' . $index . '">' . ($this->day_format == 'full' ? $day['full'] : $day['initials']) . '</th>';
        }
        ...
        do {
            ...
            $calendar .= '<td class="px-3 py-0 day cal-day cal-day-' . $day . ' cal-' . $day . ' ' . $class . $today_class . '" title="' . htmlentities(strip_tags($event_summary)) . '">';
