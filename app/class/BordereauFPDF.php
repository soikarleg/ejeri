<?php
class BordereauFPDF extends FPDF
{
    private $numero;
    private $secteur;
    private $chemin;
    ...
    private function dataHeader($secteur, $numero)
    {
        ...
        $this->factureNumber = mb_convert_encoding('réf : ' . $numero, 'ISO-8859-1');
    ...
    function Header()
    {
        ...
        $this->pdf->Cell(0, 9,  'Bordereau de remise', '', 1, 'R');
    ...
    public function Lignes($numbdx)
    {
        $reg = new Reglements($this->secteur);
        $lignes = $reg->askBordereau($numbdx);
        foreach ($lignes as $d) {
            $date = $d['jour'] . '/' . $d['mois'] . '/' . $d['annee'];
            $qui = NomConn($d['commercial']);
        }
        // var_dump($lignes);
        $desi = mb_convert_encoding('Client', 'ISO-8859-1');
        $titre = mb_convert_encoding($lignes['client'], 'ISO-8859-1');
        $titre = $titre === '' ? strEncoding('Remise réf : ' . $numbdx . ' déposée, le ' . $date . ' par ' . $qui) : $titre;
        $this->pdf->SetFont('Nunito', 'B', 9);
        $this->pdf->Cell(190, 4 + 2, $titre, 'B', 1, 'L');
        $this->pdf->Ln(4);
        $this->pdf->Cell(90, 4 + 1, $desi, 0, 0, 'L');
        $this->pdf->Cell(25, 4 + 1, 'Facture', 0, 0, 'L');
        $this->pdf->Cell(25, 4 + 1, 'Partiel', 0, 0, 'R');
        $this->pdf->Cell(25, 4 + 1, 'Acompte', 0, 0, 'R');
        $this->pdf->Cell(25, 4 + 1, 'Montant', 0, 1, 'R');
        $this->pdf->Ln(5);
        $totrem = 0;
        $tot = 0;
        foreach ($lignes as $l) {
            ...
            $desi = mb_convert_encoding($l['id'] . ' - ' . NomClient($l['id']), 'ISO-8859-1');
            ...
            //     $largeurWW = 105;
            // }
            $d = $this->pdf->WordWrap($desi, 90);
            //$this->pdf->MultiCell(115,4,$d,0,'L',0);
            //$this->pdf->Cell(95, 4 + 1, $d, 0, 0, 'L');
            $this->pdf->Write(4 + 1, $d);
            $this->pdf->SetX(100);
            $this->pdf->Cell(25, 4 + 1, $l['factref'], 0, 0, 'L');
            $this->pdf->Cell(25, 4 + 1, $l['partiel'], 0, 0, 'R');
            $this->pdf->Cell(25, 4 + 1, $l['acompte'], 0, 0, 'R');
            $this->pdf->Cell(25, 4 + 1, $tot = Dec_2($l['montant']), 0, 1, 'R');
            $this->pdf->Ln(3);
            $totrem = $tot + $totrem;
        }

        $this->pdf->SetFont('Nunito', 'B', 9);
        $this->pdf->Cell(90, 4 + 1, 'Total remise', 0, 0, 'L');
        $this->pdf->Cell(25, 4 + 1, '', 0, 0, 'L');

        $this->pdf->Cell(25, 4 + 1, '', 0, 0, 'R');
        $this->pdf->Cell(25, 4 + 1, '', 0, 0, 'R');
        $this->pdf->Cell(25, 4 + 1, Dec_2($totrem), 0, 1, 'R');
        $this->pdf->SetFont('Nunito', '', 9);
