...
'41210': { secteur: 'sologne', nom: 'Neung-sur-Beuvron' }, // Neung-sur-Beuvron
'41230': { secteur: 'sologne', nom: 'Soings-en-Sologne' }, // Soings-en-Sologne
'41250': { secteur: 'sologne', nom: 'Bracieux' }, // Bracieux
'41300': { secteur: 'sologne', nom: 'Salbris' }, // Salbris
'41350': { secteur: 'sologne', nom: 'Huisseau-sur-Cosson' },
'41500': { secteur: 'orleans', nom: 'Mer' }, // Mer
'41600': { secteur: 'sologne', nom: 'Lamotte-Beuvron' }, // Lamotte-Beuvron
...
const equipesParSecteur = {
    'orleans': {
        nom: 'EJERI Jardins - Lailly-en-Val',
        ...
        zone: 'Lailly-en-Val et un rayon de 25km (Loiret 45, Loir-et-Cher 41)',
        specialites: ['Tonte de pelouses', 'Taille de haies', 'Taille d\'arbustes', 'Remise en état','Désherbage', 'Entretien de jardins'],
        image: 'assets/img/enooki_photos/lailly.png'
    },
        'sologne': {
        nom: 'EJERI Jardins - Lamotte-Beuvron',
        description: 'Équipe spécialisée dans l\'entretien de jardins en région Centre-Val de Loire',
        contact: {
            nom: 'François',
            telephone: '02 38 45 15 78',
            email: 'lancelot@ejeri.fr',
            adresse: ''
        },
        zone: 'Lamotte-Beuvron et un rayon de 25km (Loiret 45, Loir-et-Cher 41)',
        specialites: ['Tonte de pelouses', 'Taille de haies', 'Taille d\'arbustes', 'Remise en état','Désherbage', 'Entretien de jardins'],
        image: 'assets/img/enooki_photos/lamotte.png'
    },
    'bordeaux': {
    ...
    'nantes': {
        ...
        zone: 'Clisson et un rayon de 25km (Loire-Atlantique 44, Maine-et-Loire 49, Vendée 85)',
        specialites: ['Tonte de pelouses', 'Taille de haies', 'Taille d\'arbustes', 'Remise en état','Désherbage', 'Entretien de jardins','Jardins côtiers', 'Grands espaces verts', 'Tonte et débroussaillage', 'Jardins de prestige'],
        image: 'assets/img/enooki_photos/clisson.png'
    },
    'general': {
        nom: 'Équipe hors zone',
        description: 'Notre équipe principale pour tous types d\'interventions',
        contact: {
            nom: 'EJERI Jardins - Service client',
            telephone: '02.38.45.15.78',
            email: 'francois@ejeri.fr',
            adresse: '3 place de l\'Eglise, 45740 Lailly-en-Val'
        },
        zone: 'Régions limitrophes du 41, 45, 49, 85',
        specialites: ['Equipe hors zone', 'Devis gratuit', 'Intervention à la journée'],
        image: 'assets/img/enooki_photos/tonte.jpg'
    }
};
...
function afficherEquipe(equipe, codePostal) {
    ...
    const html = `
        ...
        <div class="card card-horizontal border-0 myshadow">
            <img src="${equipe.image}" class="card-img-left" alt="${equipe.nom}">
            <div class="card-body">
                <h4 class="text-bold">${equipe.nom}</h4>
                <p class="text-muted mb-4">${equipe.description}</p>
                
                
                <div class="mb-3">
                    <h6>Zone d'intervention</h6>
                    <p class="text-muted small">${equipe.contact.adresse}</p>
                ...
                <div class=" mt-3">
                    <button class="devis-btn me-2" onclick="demanderDevis('${codePostal}')">
                        Demande de devis
                    </button>
                    <button class="devis-btn" onclick="ouvrirContact('${equipe.contact.telephone}')">
                        Appeler
                    </button>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const codePostalInputBas = document.getElementById('code-postal-bas');
    const btnRechercher = document.getElementById('btn-rechercher');
    
    // Gestion du clic sur les boutons rechercher (il peut y en avoir plusieurs)
    const boutonsRechercher = document.querySelectorAll('#btn-rechercher, button[id="btn-rechercher"]');
    boutonsRechercher.forEach(bouton => {
        bouton.addEventListener('click', function() {
            // Déterminer quel champ est le plus proche de ce bouton
            const inputProche = this.parentElement.querySelector('input[type="text"]');
            let codePostal = '';
            
            if (inputProche && inputProche.value.trim()) {
                codePostal = inputProche.value.trim();
            } else {
                // Fallback: chercher dans tous les champs
                if (codePostalInput && codePostalInput.value.trim()) {
                    codePostal = codePostalInput.value.trim();
                } else if (codePostalInputBas && codePostalInputBas.value.trim()) {
                    codePostal = codePostalInputBas.value.trim();
                }
            }
    ...
    // Gestion de la touche Entrée et validation pour code-postal
    if (codePostalInput) {
        codePostalInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                rechercherEquipeParCodePostal(this.value.trim());
            }
        });
    ...
    // Gestion de la touche Entrée et validation pour code-postal-bas
    if (codePostalInputBas) {
        codePostalInputBas.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                rechercherEquipeParCodePostal(this.value.trim());
            }
        ...
        // Validation en temps réel
        codePostalInputBas.addEventListener('input', function() {
            // Limiter à 5 chiffres
            this.value = this.value.replace(/\D/g, '').substring(0, 5);
        });
    }
