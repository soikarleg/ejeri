    ...
    <div>
        ...
        <?php if (!empty($prenom) && !empty($nom)) : ?>
            <p>
                ...
                <br>
                <a href="/modifier_contact?action=modifie_nom&id=<?= htmlspecialchars($_SESSION['client_id']) ?>" class="small">Modifier</a>
            </p>

        <?php else : ?>
            <p>Aucun nom et prénom<br>
                <a href="/modifier_contact?action=ajoute_nom" class="small">Ajouter votre nom</a>
            </p>
        <?php endif; ?>
            ...
            <?php foreach ($adresses as $adresse) : ?>
                <?php if (isset($adresse['ligne1'], $adresse['cp'], $adresse['ville'], $adresse['pays'])) : ?>
                    <p>
                        ...
                        <a href="/modifier_contact?action=modifie_adresse&id=<?= htmlspecialchars($adresse['id']) ?>" class="small">Modifier</a>
                    </p>
                <?php else : ?>
        ...
        <?php endif; ?>
        <a href="/modifier_contact?action=ajoute_adresse&id=<?= htmlspecialchars($_SESSION['client_id']) ?>" class="small">Ajouter une adresse</a><br>
        ...
        <?php else : ?>
            <p class="mt-4">Aucun téléphone enregistré.<br>
                <a href="/modifier_contact?action=ajoute_telephone&id=<?= htmlspecialchars($_SESSION['client_id']) ?>" class="small">Ajouter votre numéro de téléphone</a>
            </p>
        <?php endif; ?>
